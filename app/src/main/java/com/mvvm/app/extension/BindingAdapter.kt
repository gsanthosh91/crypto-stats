...
import com.mvvm.app.ui.adapter.CoinAdapter
import com.mvvm.app.ui.adapter.UserAdapter
...
fun setItems(view: RecyclerView, items: List<User>?) {
    view.adapter = UserAdapter().apply {
...
@BindingAdapter("bind:items")
fun setCoins(view: RecyclerView, items: List<Coin>?) {
    view.adapter = CoinAdapter().apply {
        itemList = items ?: emptyList()
        notifyDataSetChanged()
    }
}
